...
ServicesCollector::Result ServicesCollector::Collect(std::uint32_t limit) {
  Result out;
  ...
  for (DWORD i = 0; i < services_returned; ++i) {
    ...
    ipc::InventoryServiceEntry entry;
    ...
    if (svc != nullptr) {
      ...
      if (GetLastError() == ERROR_INSUFFICIENT_BUFFER && cfg_needed > 0) {
        ...
        if (QueryServiceConfigW(svc, cfg, cfg_needed, &cfg_needed)) {
          ...
          entry.account =
              wevt::WideToUtf8(cfg->lpServiceStartName ? cfg->lpServiceStartName
                                                       : L"");
          entry.binary_path =
  ...
  if (out.truncated || config_gaps) {
    ...
    if (out.truncated && config_gaps) {
      out.status_detail = "Service list truncated and some configs unavailable";
    } else if (out.truncated) {
      out.status_detail = "Service list truncated at collector limit";
    } else {
      out.status_detail = "Some service configs were unavailable";
    }
  } else {
